...
from trojai_mitigation_round.trojai_dataset import Round11SampleDataset, Round11AllModelDataset
...
def prepare_model(path, device):
    ...
    return model, model.state_dict()['fc.bias'].shape[0]


def prepare_dataset(dataset_path, n_cls, split_name, train_mode='round', clean_only=False):
    print(dataset_path)
    require_label = True
    if split_name == 'train':
        if train_mode == 'round':
            print('loading samples for all models')
            dataset = Round11AllModelDataset(root=dataset_path, n_cls=n_cls, split=split_name, require_label=require_label, clean_only=clean_only)
        else:
            print('loading only small batch of samples')
...
def run_mitigate_mode(args):
    ...
    model, n_cls = prepare_model(args.model_filepath, args.device)
    mitigation = prepare_mitigation(args, config_json)

    model_dirpath = os.path.dirname(args.dataset_dirpath)
    if os.path.exists(os.path.join(model_dirpath, 'new-clean-example-data')):
        use_dirpath = model_dirpath
        print('Use assumption path as path')
    else:
        use_dirpath = args.round_training_dataset_dirpath
        print('Use roundtraining path as path')
    # dataset1 = prepare_dataset(use_dirpath, n_cls, split_name='train')
    # dataset2 = prepare_dataset(args.dataset_dirpath, n_cls, split_name='train', train_mode='small')
    # dataset2 = prepare_dataset(use_dirpath, n_cls, split_name='train', clean_only=True)
    
    # mitigate_model(model, mitigation, (dataset1, dataset2), args.output_dirpath, args.model_output_name)
    
    # dataset1 = prepare_dataset(use_dirpath, n_cls, split_name='train')
    dataset2 = prepare_dataset(args.dataset_dirpath, n_cls, split_name='train', train_mode='small')
    # dataset2 = prepare_dataset(use_dirpath, n_cls, split_name='train', clean_only=True)
    
    mitigate_model(model, mitigation, (dataset2), args.output_dirpath, args.model_output_name)
...
def run_test_mode(args):
    ...
    model, n_cls = prepare_model(args.model_filepath, args.device)
    mitigation = prepare_mitigation(args, config_json)
    dataset = prepare_dataset(args.dataset_dirpath, n_cls, split_name='test')
